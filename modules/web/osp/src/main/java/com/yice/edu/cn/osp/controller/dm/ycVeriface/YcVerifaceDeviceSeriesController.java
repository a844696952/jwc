...
public class YcVerifaceDeviceSeriesController {
    ...
    @PostMapping("/findYcVerifaceCameraDeviceSeriesListByCondition")
    @ApiOperation(value = "根据条件查找摄像头系列列表", notes = "返回响应对象,不包含总条数", response=YcVerifaceDeviceSeries.class)
    public ResponseJson getYcVerifaceCameraDeviceSeriesListByCondition(
            @ApiParam(value = "属性不为空则作为条件查询")
            @RequestBody YcVerifaceDeviceSeries ycVerifaceDeviceSeries){
        ycVerifaceDeviceSeries.setSeriesType("0");
        List<YcVerifaceDeviceSeries> data=ycVerifaceDeviceSeriesService.findYcVerifaceDeviceSeriesListByCondition(ycVerifaceDeviceSeries);
        return new ResponseJson(data);
    }

    @PostMapping("/findYcVerifaceDoorDeviceSeriesListByCondition")
    @ApiOperation(value = "根据条件查找门禁系列列表", notes = "返回响应对象,不包含总条数", response=YcVerifaceDeviceSeries.class)
    public ResponseJson getYcVerifaceDoorDeviceSeriesListByCondition(
            @ApiParam(value = "属性不为空则作为条件查询")
            @RequestBody YcVerifaceDeviceSeries ycVerifaceDeviceSeries){
        ycVerifaceDeviceSeries.setSeriesType("1");
        List<YcVerifaceDeviceSeries> data=ycVerifaceDeviceSeriesService.findYcVerifaceDeviceSeriesListByCondition(ycVerifaceDeviceSeries);
        return new ResponseJson(data);
    }
