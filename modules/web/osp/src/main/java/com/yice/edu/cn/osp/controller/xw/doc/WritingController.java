...
public class WritingController {
    ...
    @GetMapping("/update/findWritingById/{id}")
    @ApiOperation(value = "去更新页面,通过id查找", notes = "返回响应对象")
    public ResponseJson findWritingById(
    ...
    @ApiOperation(value = "根据条件查找,返回我发送的发文列表 （条件:当前状态（writingType字段 0为全部 1为未审核 2为已完成 3待修改（驳回）  时间段（SearchTimeZone[]字段） 公文类型（writingNumberType字段 1为公文  2为教育督导）））", notes = "返回响应对象")
    ...
    public ResponseJson findOneWritingByCondition(
            @ApiParam(value = "属性不为空则作为条件查询")
            @RequestBody Writing writing){
        Writing one=writingService.findOneWritingByCondition(writing);
        return new ResponseJson(one);
    }
    @GetMapping("/deleteWriting/{id}")
    @ApiOperation(value = "根据id删除", notes = "返回响应对象")
    public ResponseJson deleteWriting(
    ...
    public ResponseJson findWritingListByCondition(
            @ApiParam(value = "属性不为空则作为条件查询")
            @Validated
            @RequestBody Writing writing){
        writing.setSchoolId(LoginInterceptor.mySchoolId());
    ...
    public FileObj uploadHandout(MultipartFile file){
        ...
        try{
            ...
            fileObj.setPath(QiniuUtil.uploadDocFile(file, Constant.Upload.UPLOAD_WRITING));
        }catch (Exception e){
