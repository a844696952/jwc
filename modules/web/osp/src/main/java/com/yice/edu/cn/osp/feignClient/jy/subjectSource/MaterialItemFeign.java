...
@FeignClient(value="jy",contextId = "materialItemFeign",path = "/materialItem")
public interface MaterialItemFeign {
    ...
    List<KnowledgePoint> findKnowledgePointListByItem(MaterialItemKnowledge materialItemKnowledge);
    @PostMapping("/findKnowledgePointCountByItem")
    long findKnowledgePointCountByItem(MaterialItemKnowledge materialItemKnowledge);
    ...
    void batchSaveMaterialItem(@RequestBody List<MaterialItem> materialItems);
    @PostMapping("/findChapterTree")
    ResponseJson findChapterTree(ResourceVo resourceVo);
}
