...
import com.yice.edu.cn.yed.service.jy.titleQuota.ResourcePlatformService;
...
public class ResourcePlatformController {
    ...
    @GetMapping("/update/findResourcePlatformById/{id}")
    @ApiOperation(value = "去更新页面,通过id查找运营平台（题库资源平台）", notes = "返回响应对象", response=ResourcePlatform.class)
    public ResponseJson findResourcePlatformById(
            @ApiParam(value = "去更新页面,需要用到的id", required = true)
            @PathVariable String id){
        ResourcePlatform resourcePlatform=resourcePlatformService.findResourcePlatformById(id);
        return new ResponseJson(resourcePlatform);
    }

    @PostMapping("/update/updateResourcePlatformForAll")
    @ApiOperation(value = "修改运营平台（题库资源平台）对象所有字段", notes = "返回响应对象")
    public ResponseJson updateResourcePlatformForAll(
            @ApiParam(value = "被修改的运营平台（题库资源平台）对象", required = true)
            @RequestBody ResourcePlatform resourcePlatform){
        resourcePlatform = resourcePlatformService.updateResourcePlatformForAll(resourcePlatform);
        return new ResponseJson(resourcePlatform);
    }
}
