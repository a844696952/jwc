...
import com.yice.edu.cn.pcd.service.pcdDoc.PcdDocRevertService;
...
public class PcdDocRevertController {
    ...
    public ResponseJson savePcdDocRevert(
        ...
        pcdDocRevert.setEehId(LoginInterceptor.currentEehId());
        pcdDocRevert.setEehName(LoginInterceptor.currentAccount().getEehName());
        pcdDocRevert.setName(LoginInterceptor.currentAccount().getName());
        pcdDocRevert.setCreateUserId(LoginInterceptor.currentAccount().getId());
        pcdDocRevertService.savePcdDocRevertKong(pcdDocRevert);
        return new ResponseJson();
    }

    @PostMapping("/findPcdDocRevertById")
    ...
    public ResponseJson findPcdDocRevertById(
            ...
            @RequestBody PcdDocRevert pcdDocRevert){
        List<PcdDocRevert> pcdDocRevert1 =pcdDocRevertService.findAndUpdatePcdDocRevertById(pcdDocRevert);
        return new ResponseJson(pcdDocRevert1);
    }
    ...
    public ResponseJson findPcdDocRevertsByCondition(
            ...
            @Validated
            @RequestBody PcdDocRevert pcdDocRevert){
        List<PcdDocRevert> data=pcdDocRevertService.findPcdDocRevertByPcdDocRevertListKong(pcdDocRevert);
        long count=pcdDocRevertService.findPcdDocRevertByPcdDocRevertLongKong(pcdDocRevert);
