...
import com.yice.edu.cn.ecc.service.parentMsg.DmParentQuickReplyService;
...
public class DmParentQuickReplyController {
    ...
    @PostMapping("/getDmParentQuickReplyListByCard/{id}")
    @ApiOperation(value = "根据学生的卡号，获取到家长的快捷回复", notes = "返回响应对象,不包含总条数", response=DmParentQuickReply.class)
    public ResponseJson getDmParentQuickReplyListByCard(
            @ApiParam(value = "属性不为空则作为条件查询")
            @Validated
            @PathVariable("id") String id){
        List<DmParentQuickReply> dmParentQuickReplyList = dmParentQuickReplyService.getDmParentQuickReplyListByCard(id);
        if(dmParentQuickReplyList==null){
            return new ResponseJson(false,"无快捷回复");
        }else{
            return new ResponseJson(dmParentQuickReplyList);
        }
    }
