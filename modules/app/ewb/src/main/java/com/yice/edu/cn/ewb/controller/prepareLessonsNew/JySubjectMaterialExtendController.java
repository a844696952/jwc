...
import com.yice.edu.cn.ewb.service.prepareLessonsNew.JySubjectMaterialExtendService;
...
public class JySubjectMaterialExtendController {
    ...
    public ResponseJson findJySubjectMaterialExtendListByCondition(
            ...
            @Validated
            @RequestBody JySubjectMaterialExtend jySubjectMaterialExtend) {
        if (jySubjectMaterialExtend.getType() != null) {
            jySubjectMaterialExtend.setSchoolId(mySchoolId());
            List<JySubjectMaterialExtend> data = jySubjectMaterialExtendService.findJySubjectMaterialExtendListByCondition(jySubjectMaterialExtend);
            return new ResponseJson(data);
        } else {
            MaterialItem materialItem = new MaterialItem();
            materialItem.setParentId(jySubjectMaterialExtend.getParentId());
    ...
    public ResponseJson findJySubjectMaterialExtendTreeByTextBookId(
        ...
        if(materialItemService.checkMaterialItemIsNull(jySubjectMaterialExtend.getMaterialId()) == false){
            return new ResponseJson(false,"当前章节不存在");
        }
    ...
    @ApiOperation(value = "根据章节id查询该章节是否存在", notes = "返回布尔值")
    public ResponseJson checkMaterialItemIsNull(
            @ApiParam(value = "章节id", required = true)
            @PathVariable String materialId){
        if(materialItemService.checkMaterialItemIsNull(materialId)){
            return new ResponseJson(true,"该章节存在");
        }else {
            return new ResponseJson(false,"当前章节已不存在");
        }
    }
