package com.yice.edu.cn.tap.controller.prepLessonResource;
...
import com.yice.edu.cn.tap.service.prepLessonResource.PrepLessonResourceFileService;
...
@Api(value = "/prepLessonResourceFile",description = "文档图片模块")
...
*/
    ...
    @ApiOperation(value = "默认查找所有类型文件，fileType文件类型/meterialItemId章节id/filename文件名,分页需要传pager", notes = "返回响应对象")
    public ResponseJson findMatFileByMatItemid(
            @ApiParam(value = "去更新页面,需要用到的教材章节id", required = true)
            @RequestBody PrepLessonResourceFile prepLessonResourceFile) {
        prepLessonResourceFile.setRole("0");
        School school =schoolService.findSchoolById(mySchoolId());
        prepLessonResourceFile.setAnnualPeriodId(school.getTypeId());
