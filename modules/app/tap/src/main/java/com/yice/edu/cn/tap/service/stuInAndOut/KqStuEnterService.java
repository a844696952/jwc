...
import com.yice.edu.cn.tap.feignClient.stuInAndOut.StuStatusChangeFeign;
...
public class KqStuEnterService {
    ...
    public void sendStuNowStatusToParent(Student student) {
       String studentNowStatus= student.getNowStatus()!=null?student.getNowStatus():"";
        String[] stuId = new String[]{student.getId()};
        if (studentNowStatus.equals(Constant.STUDENT_NOW_STATUS_TYPE.IN_SCHOOL)){
            final Push push = Push.newBuilder(JpushApp.BMP).getSimplePush(stuId,"出入校通知","您的孩子已经安全到校！（此条消息由班主任推送）", Extras.XW_STUDENT_IN_OUT_SCHOOL);
            stringRedisTemplate.convertAndSend(Constant.MCS_CHANEL.JPUSH_SEND_PUSH, JSONUtil.toJsonStr(push));
        }else if (studentNowStatus.equals(Constant.STUDENT_NOW_STATUS_TYPE.NOT_INT_SCHOOL)){

            final Push push = Push.newBuilder(JpushApp.BMP).getSimplePush(stuId,"出入校通知","您的孩子已经离开学校！（此条消息由班主任推送）", Extras.XW_STUDENT_IN_OUT_SCHOOL);
            stringRedisTemplate.convertAndSend(Constant.MCS_CHANEL.JPUSH_SEND_PUSH, JSONUtil.toJsonStr(push));
        }else if (studentNowStatus.equals(Constant.STUDENT_NOW_STATUS_TYPE.LEAVE_IN)||studentNowStatus.equals(Constant.STUDENT_NOW_STATUS_TYPE.LEAVE_OUT)){
            final Push push = Push.newBuilder(JpushApp.BMP).getSimplePush(stuId,"出入校通知","您的孩子请假已经通过审批！（此条消息由班主任推送）", Extras.XW_STUDENT_IN_OUT_SCHOOL);
            stringRedisTemplate.convertAndSend(Constant.MCS_CHANEL.JPUSH_SEND_PUSH, JSONUtil.toJsonStr(push));
        }
        //添加学生在校状态变化记录
