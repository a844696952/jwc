...
import com.yice.edu.cn.tap.service.courseware.CourseResService;
...
//        if(courseRes.getResType().equals(ResType.MATERIAL)){
//            if(!ResType.MATERIAL.matches(courseRes.getFileType())){
//                return new ResponseJson("上传的素材文件格式不符合规范");
//            }
//        }
        ...
        if(courseRes.notMatchType()){
            return new ResponseJson(false,"资源格式和资源类型不匹配");
        }
...
//
    ...
    public ResponseJson findCourseResListByCondition(
            ...
            @RequestBody CourseRes courseRes){
        if(courseRes.getResType()==null){
            return new ResponseJson(false,"ResType不可为空");
        }
//        if(courseRes.getFileType()==null){
//            return new ResponseJson(false,"FileType不可为空");
//        }
        courseRes.setSchoolId(mySchoolId());
        courseRes.setTeacherId(myId());
...
//
    ...
    public ResponseJson findCourseResPageByCondition(
            @ApiParam(value = "属性不为空则作为条件查询")
            @RequestBody CourseRes courseRes){
        if(courseRes.getPager()==null){
            return new ResponseJson(false,"pager不可为空");
        }
        if(courseRes.getPager().getPage()<1){
            return new ResponseJson(false,"pager.page不可小于1");
        }
        if(courseRes.getPager().getPageSize()<=0 || courseRes.getPager().getPageSize()>100){
            return new ResponseJson(false,"pager.pageSize需要大于0，小于100");
        }
        if(courseRes.getResType()==null){
            return new ResponseJson(false,"ResType不可为空");
        }
...
//            return new ResponseJson(false,"FileType不可为空");
//        }
        CourseRes courseResParam = new CourseRes();
        courseResParam.setSchoolId(mySchoolId());
        courseResParam.setTeacherId(myId());
        courseResParam.setPager(courseRes.getPager());
        List<CourseRes> data=courseResService.findCourseResListByCondition(courseRes);
...
//
    ...
    @ApiOperation(value = "单个资源备注修改", notes = "返回单个课件资源表,没有时为空")
    @PostMapping("/remark")
    public ResponseJson remark(
            @ApiParam(value = "单个资源备注修改")
            @RequestBody CourseRes courseRes
    ){
        if(StringUtils.isBlank(courseRes.getId())){
            return new ResponseJson(false,"id不可为空");
        }
