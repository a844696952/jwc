...
@ComponentScan("com.yice.edu.cn.cc.controller")
@EnableSwagger2
public class SwaggerConfig {
    ...
    public Docket cloudCourseApi(){
        return new Docket(DocumentationType.SWAGGER_2)
                ...
                .pathProvider(swaggerPathProvider)
                .select()
                .paths(cloudCoursePaths())
                .build();
    }

    private Predicate<String> cloudCoursePaths() {
        return or(
                regex("/cloudCourse/.*"),
                regex("/cloudCourseShareFile/.*"),
//                regex("/h5nl/cloudCourse/.*"),
                regex("/srsEvent/.*")
    ...
    @Bean
    public Docket h5CloudCourseApi(){
        return new Docket(DocumentationType.SWAGGER_2)
                .groupName("H5云课程模块")
                .apiInfo(apiInfo())
                .pathProvider(swaggerPathProvider)
                .select()
                .paths(h5CloudCoursePaths())
                .build();
    }

    private Predicate<String> h5CloudCoursePaths() {
        return or(
                regex("/h5nl/cloudCourse/.*")
        );
    }
