...
public interface ITeacherClassesDao {
    ...
    TeacherClasses findTeacherClassesById(String id);

    void saveTeacherClasses(TeacherClasses teacherClasses);

    void updateTeacherClasses(TeacherClasses teacherClasses);

    void deleteTeacherClasses(String id);
     ...
     * @return
     */
    List<Map<String,Object>> findTeacherClassPostCourseList(TeacherClasses teacherClasses);

    List<TeacherClassesForQusSurvey> findClassesByTeacherInfo(TeacherClassesForQusSurvey teacher5Classes);

    List<TeacherClassesForStuEvaluate> findClassesHeadTeacherBySchoolId(@Param("schoolId") String schoolId);
    ...
    @Select("SELECT DISTINCT c.`id` AS courseId,c.`alias` AS courseName,c.`name` AS subjectName FROM jw_course c " +
            ...
            "INNER JOIN jw_classes jc ON jc.`id` = tc.`classes_id`" +
            "INNER JOIN ad_dd dd ON dd.`id` = c.`id`" +
            "WHERE c.exam = 1 AND dd.`ext1` = 1 AND tc.`teacher_id` = #{teacherId}  ")
    @ResultType(value = TeacherClassesCourse.class)
    ...
    */
     ...
     * @param collect
     */
    void deleteTeacherClassesByTeachers(List<String> collect);
}
