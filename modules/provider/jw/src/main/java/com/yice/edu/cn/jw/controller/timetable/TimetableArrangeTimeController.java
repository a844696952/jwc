...
import com.yice.edu.cn.jw.service.timetable.TimetableArrangeTimeService;
...
public class TimetableArrangeTimeController {
    ...
    @GetMapping("/findTimetableArrangeTimeById/{id}")
    @ApiOperation(value = "通过id查找教师、课程不排课时间条件", notes = "返回教师、课程不排课时间条件对象")
    public TimetableArrangeTime findTimetableArrangeTimeById(
            @ApiParam(value = "需要用到的id", required = true)
            @PathVariable String id){
        return timetableArrangeTimeService.findTimetableArrangeTimeById(id);
    }
    ...
    public ResponseJson saveTimetableArrangeTime(
            ...
            @RequestBody @Valid List<TimetableArrangeTime> timetableArrangeTimes){
        
        return timetableArrangeTimeService.saveTimetableArrangeTime(timetableArrangeTimes);
    }
