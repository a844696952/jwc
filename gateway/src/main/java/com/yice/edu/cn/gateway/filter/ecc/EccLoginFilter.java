package com.yice.edu.cn.gateway.filter.ecc;
 ...
 */
@Component
public class EccLoginFilter extends AbstractGatewayFilterFactory {
    Pattern pattern = Pattern.compile("^/ecc/dmClassCard/dmClassCardLog|^/ecc$|\\..*|.*/swagger.*|.*/webjars.*|.*/v2/api-docs.*$");
    @Override
            ...
            if(tokens==null||tokens.size()==0){
                ...
                if(claims!=null){
                    ...
                    try{
                        ...
                        if(Objects.isNull(jsonObject) || jsonObject.size()<1){
                            responseJson=new ResponseJson(false,Constant.HAVEN_LOGIN,"解析token id失败");
                        }
                    }catch (Exception ex){
                        responseJson=new ResponseJson(false,Constant.HAVEN_LOGIN,"解析token id失败");
                    }
                    request=exchange.getRequest().mutate().header(Constant.Redis.ECC_TEACHER_ID_HEADER, id).build();
                }
            }
