package com.yice.edu.cn.gateway.filter.ewb;
 ...
 */
@Component
public class EwbLoginFilter extends AbstractGatewayFilterFactory {
    Pattern pattern = Pattern.compile("^/ewb/login/*|^/ewb$|\\..*|.*/swagger.*|.*/webjars.*|.*/v2/api-docs.*$");
    @Override
        ...
        return (exchange, chain) -> {
            ...
            if(tokens==null||tokens.size()==0){
                ...
                if(claims!=null){
                    ...
                    if(subject.equals("{}")){
                        request=exchange.getRequest().mutate().header(Constant.Redis.EWB_TEACHER_ID_HEADER, id).build();
                    }else{
                        request=exchange.getRequest().mutate().header(Constant.Redis.EWB_STUDENT_ID_HEADER, id).build();
                    }
